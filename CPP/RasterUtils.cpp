...
namespace {
    Raster _readRasterUncompressed(const std::string& file_path){
        ...
        printf("Loaded %d x %d rows x cols\n", num_rows, num_cols);

        if(num_rows <= 0 || num_cols <= 0){
            throw std::runtime_error("Error in file formate, rows and cols should be positive integers");
        }
        ...
        while(ifs >> c){
            ...
            if(c == '1'){
                r.setBit(column_offset, row_counter*32 + row_offset);
            }
...
// TODO - come fix these with 
//  get bit and set bit
//  but they never finish anyway
//  so not a super priority
// void bakeRaster_cpu(
